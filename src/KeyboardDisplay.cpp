...
std::unordered_map<std::string, std::stack<Track::TrackColor>> m_key_colors;
	...
	int key_space, int x_offset, int y_offset) const
{
	...
	{
		const std::string note_name = STRING(current_white << current_octave);

		auto it = m_key_colors.find(note_name);
		bool active = (it != m_key_colors.end() && !it->second.empty());

		Color c = white;
		if (active)
		{
			c = Track::ColorNoteWhite[it->second.top()];
		}
	...
	const std::vector<Track::Properties>& track_properties) const
{
	...
	const static int NotesPerOctave = 12;
	const static int WhiteNotesPerOctave = 7;
	...
	const static int MinNoteHeight = 1;

	// First pass: Draw white notes
	for (TranslatedNoteSet::const_iterator i = notes.begin(); i != notes.end(); ++i)
	{
		...
		if (is_black) continue; // Skip black notes in this pass

		const int octave_offset = (std::max(octave - 1, 0) * WhiteNotesPerOctave);
		...
		const long long adjusted_start = std::max(i->start - current_time, -roll_under);
		const long long adjusted_end = std::max(i->end - current_time, 0LL);
		...
		const int start_x = (octave_offset + inner_octave_offset) * (white_width + key_space) + x_offset;

		const int left = start_x - 1;
		const int top = y_start;
		const int width = white_width + 2;
		...
		DrawNote(renderer, tex_white, WhiteNoteDimensions, left, top, width, height, brush_id);
	}

	// Second pass: Draw black notes
	for (TranslatedNoteSet::const_iterator i = notes.begin(); i != notes.end(); ++i)
	{
		if (i->start > current_time + show_duration) break;
		...
		if (!is_black) continue; // Skip white notes in this pass
		...
		const int start_x = (octave_offset + inner_octave_offset) * (white_width + key_space) + black_offset + x_offset;

		const int left = start_x - 1;
		const int top = y_start;
		const int width = black_width + 2;
		int height = y_end - y_start;
		...
		DrawNote(renderer, tex_black, BlackNoteDimensions, left, top, width, height, brush_id);
	}
}


void KeyboardDisplay::SetKeyActive(const std::string& key_name, bool active, Track::TrackColor color)
{
	if (active)
	{
		m_key_colors[key_name].push(color);
	}
	else
	{
		if (m_key_colors.find(key_name) != m_key_colors.end() && !m_key_colors[key_name].empty())
		{
			m_key_colors[key_name].pop();
			if (m_key_colors[key_name].empty())
			{
				m_key_colors.erase(key_name);
			}
		}
	}
}

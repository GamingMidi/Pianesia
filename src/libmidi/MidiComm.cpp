...
typedef UINT(WINAPI* SDLD)(DWORD);
...
static SDLD SendDirectLongData = 0;
...
void MidiCommIn::InputCallback(unsigned int msg, unsigned long p1, unsigned long)
{
	...
	catch (const MidiError& e)
	{
		...
		const static wstring error_header1 = L"Pianesia detected a";
...
MidiCommOut::MidiCommOut(unsigned int device_id)
{
	...
	if (!OM) {
		OM = LoadLibraryW(L"OmniMIDI");
		//isUsingSyndrv = false

		//if (!OM) return;
		//return MessageBox(hwndApp, "Could not load OmniMIDI, Is it installed?", "TMIDI Error", MB_ICONERROR);
		...
		{
			printf("OmniMIDI init\n");
			//return true;
		}
		else
		{
			printf("OmniMIDI failed to initialize!\n");
			//MessageBox(hwndApp, "Failed to initialize KDMAPI", "TMIDI Error", MB_ICONERROR);
		}
	}
}
...
MidiCommIn::MidiCommIn(unsigned int device_id)
{
	...
	MIDIClientCreate(CFSTR("Pianesia"), 0, this, &m_client);
	MIDIInputPortCreate(m_client, CFSTR("Pianesia In"), midi_input, this, &m_port);
...
void MidiCommOut::Acquire(unsigned int device_id)
{
	...
	{
		MIDIClientCreate(CFSTR("Pianesia"), 0, this, &m_client);
		MIDIOutputPortCreate(m_client, CFSTR("Pianesia Out"), &m_port);

...
const static wstring application_name = L"Pianesia";
const static std::wstring friendly_app_name = WSTRING(L"Pianesia " << PianoGameVersionString);
...
#endif
{
   ...
   {
      wstring wrapped_description = WSTRING(L"Pianesia detected an unknown problem and must close!  '" << e.what() << "'" << error_footer);
      Compatible::ShowError(wrapped_description);
   }
   catch (...)
   {
      wstring wrapped_description = WSTRING(L"Pianesia detected an unknown problem and must close!" << error_footer);
      Compatible::ShowError(wrapped_description);
   }
